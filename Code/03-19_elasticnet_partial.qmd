...
weather_recipe <- 
  ...
  recipe(strength_gtex ~ .,  #explain by all other columns in this dataset
         data = weather_train) %>%
  # Removing year, site, and weather outside of growing season  
  step_rm(year, site,
          matches("Jan|Feb|Mar|Apr|Nov|Dec") #will remove anything tht has this text in it
          ) #%>%
  # Normalizing
...
First, let's specify:  
  ...
  - which **engine** we want to use  (specific to the kind of model, ie penalized)

An elastic net model is a linear regression model, penalized.  

Elastic net **hyperparameters**:  
  - **penalty**: equivalent to lambda  
  - **mixture**: if 0 (a ridge) to 1 (a lasso)  
...
enet_spec  <-
  ...
  linear_reg(penalty = tune(), #no value given = asking the model to fine tune for us
             mixture = tune()
             ) %>%
  # Specify the engine
  set_engine("glmnet")
...
```{r enet_grid}
enet_grid <- crossing(penalty = seq(0, #crossing gives all potential combinations
                                    10, 
                                    by = 1), #step size can be changed
                      mixture = seq(0, 
                                    1, 
                                    by = 0.1))
...
```{r resampling_foldcv}
resampling_foldcv <- vfold_cv(weather_train, #v is the number of folds, 10 is common
                              v = 10)

resampling_foldcv
resampling_foldcv$splits[[1]]

#<438/49/487> in the first fold, there are 487 total observations, 49 of which are assessment and 438 of which are testing. The numbers may be similar, but in each iteration the actual data in each is different
```
...
```{r enet_grid_result}
enet_grid_result <- tune_grid(enet_spec, #setting the model type
                              preprocessor = weather_recipe, #the recipe
                              grid = enet_grid, #the grid
                              resample = resampling_foldcv #the fold/sampling strategy
                              )

enet_grid_result


enet_grid_result$.metrics[[1]] #242 rows, 6 columns. For that first fold, it tested across all of the grid values and each time it tested it extracted an estimate fit metric (.rmse and .estimate for the ability to predict the assessment set for this fold)
```
...
```{r RMSE}
enet_grid_result %>%
  collect_metrics() %>%  #adds two columns, including the mean of the metrics
  filter(.metric == "rmse") %>%
...
#we want LOW rmse (this is the error value). the reddish line is the best model
```
...
#in this case, the highest R2 is better. the best is 
```
What penalty and mixture values created lowest RMSE?  
...
best_rmse <- enet_grid_result %>%
  select_best(metric = "rmse") #for rmse above

best_rmse

#best has a penality of 1 and a mixture of 0
```

```{r}
# Based on greatest R2
best_r2 <- enet_grid_result %>%
  select_best(metric = "rsq") #for r2

best_r2

#best model has penalty 1 but mixture of 0.1
```
...
final_fit %>%
  collect_predictions() #getting predictions on the model ran on the ENTIRE training set, but predictions based on the test set (211 rows)
```
...
final_fit %>%
  ...
  ggplot(aes(x = strength_gtex,
             y = .pred)) + #remember that this is only on the test set
  geom_point() +
  geom_abline() + #1:1 line added to plot
  geom_smooth(method = "lm") +
  scale_x_continuous(limits = c(20, 40)) +
  scale_y_continuous(limits = c(20, 40)) 

#where they cross, above a certain point the predictions become not great
```
...
final_spec %>%
  ...
  tidy() %>%
  arrange(desc(estimate)) #will be drom high to low contributions
...
final_spec %>%
  fit(strength_gtex ~ .,
         data = bake(weather_prep, weather)) %>%
    vi() %>% #up to this point, gives us the variable, its coefficient, and whether that is positive or negative
  mutate(
...
#this graph gives the top variables and whether they have a positive or negative effect on the prediction
```
